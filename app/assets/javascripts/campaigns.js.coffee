  ...
  submitPaymentForm: (form) ->
    ...
    if !$.isEmptyObject(errors)
      ...
      .fail((jqXHR, textStatus) ->
        Crowdhoster.campaigns.resetPaymentForm()
        if jqXHR.status == 400
          $('#errors').append('<p>Sorry, we weren\'t able to process your contribution. Please try again.</p><br><p>If you continue to experience issues, please <a href="mailto:support@altheahealth.com?subject=Support request for creating user payment">click here</a> to email support.</p>')
        else
          $('#errors').append('<p>Sorry, we weren\'t able to process your contribution. Please try again.</p><br><p>If you continue to experience issues, please <a href="mailto:support@altheahealth.com?subject=Unable to create user payment">click here</a> to email support.</p>')
      )
  ...
  cardResponseHandler: (response) ->
    ...
    switch response.status
      when 201
        ...
        $('#refresh-msg').hide()
        $('#errors').append('<p>An error occurred. Please check your credit card details and try again.</p><br><p>If you continue to experience issues, please <a href="mailto:support@altheahealth.com?subject=Support request for a payment issue&body=PLEASE DESCRIBE YOUR PAYMENT ISSUES HERE">click here</a> to contact support.</p>')
        $('#errors').show()
        $('.loader').hide()
        $button = $('button[type="submit"]')
        $button.attr('disabled', false).html('Confirm payment of $' + $button.attr('data-total') )
